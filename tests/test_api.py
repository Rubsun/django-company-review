...
from companies_app.models import Company, Equipment, Category, Review
...
def create_apitest(model_class, model_url, creation_attrs):
    class APITest(TestCase):
        ...
        def test_manage_user(self):
            self.manage(self.user, self.user_token, status.HTTP_403_FORBIDDEN, status.HTTP_403_FORBIDDEN,
                        status.HTTP_403_FORBIDDEN)

        def test_manage_superuser(self):
            self.manage(self.superuser, self.superuser_token, status.HTTP_201_CREATED, status.HTTP_200_OK,
